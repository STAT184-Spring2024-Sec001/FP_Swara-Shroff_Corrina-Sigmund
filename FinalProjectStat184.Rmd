...
heart_attack_pred_data<-heart_attack_prediction_dataset.2  %>% 
  ...
  ggplot(heart_attack_pred_data) +
  aes(x = factor(Diet), fill = Sex) +  # Convert to factor for discrete data
  geom_bar(position = "dodge", stat = "count") +  # Create a bar plot with dodged bars
  scale_fill_brewer(palette = "Set1") +  # Choose color palette
  labs(x = "Diet", y = "Number of Individuals", fill = "Sex") +  # Add axis labels and legend title
  ggtitle("Diet Type for Each Sex") +  # Add plot title
  theme_minimal()  # Choose a theme

```

## Physical Activity Days per Week V. Sex Frequency Table
